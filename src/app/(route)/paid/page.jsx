...
export default function Paid() {
  ...
  const totalPrice = reservations?.reduce(
    (acc, reservation) => acc + reservation.accommodation.room.price,
    0
  );
  ...
  return (
    <div className="container mx-auto p-8">
      <p className="text-2xl font-semibold mb-7">
        상품 정보 {reservations?.length}건
      </p>
      {reservations?.map((reservation) => (
        <div
          key={reservation.orderId}
          className="mb-5 bg-white shadow-md rounded-xl overflow-hidden">
          <div className="flex flex-col md:flex-row">
            ...
            <div className="p-6 ml-4 flex-column items-center">
              <h2 className="text-xl font-semibold mb-4">
                {reservation.accommodation.accommodationName}
              </h2>
              ...
              <div className="flex justify-between items-center">
                <div>
                  <div className="flex mb-3 text-sm">
                    ...
                    <p>
                      {dayjs(reservation.accommodation.checkIn).format(
                        "YYYY-MM-DD (ddd)"
                      )}{" "}
                      ~{" "}
                      {dayjs(reservation.accommodation.checkOut).format(
                        "YYYY-MM-DD (ddd)"
                      )}{" "}
                      |{" "}
                      {calculateNights(
                        reservation.accommodation.checkIn,
                        reservation.accommodation.checkOut
                      )}
                      박
                    </p>
                  </div>
      ...
      <div className="bg-white shadow-md rounded-lg p-6 flex justify-end">
        ...
        <p className="text-xl font-semibold text-blue-500">{totalPrice} 원</p>
      </div>
    </div>
  );
}
